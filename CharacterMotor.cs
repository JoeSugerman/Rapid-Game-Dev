...
public class CharacterMotor : MonoBehaviour 
{
	...
	void Awake()
	{
        ...
        //get audio source to update clip
	...
	{
        player.aSource.volume = player.normalSoundLevel; //determine level of volume when player is walking normally - can be adjusted in the inspector
        ...
        {          
            ...
            if (volumeTimer <= 0 && player.grounded && !player.sneaking && !player.running) //make sure volume timer has expired and that the player isn't jumping, sneaking or running so sound can play again
            {
                player.aSource.Play();
                volumeTimer = 0.2f;
            }
